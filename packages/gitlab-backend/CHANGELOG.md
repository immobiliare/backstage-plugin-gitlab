...
- **Dependency synchronization:** This release "updates all deps" to bring the plugin in line with **Backstage >= 1.48.0**.
- **Architecture Migration:** The backend plugin now fully supports the New Backend System. If you were using `createRouter` manually, ensure you use the `httpRouter` service instead.
- **Example Migration (Old Backend):**

  ```typescript
  await createRouter({ logger, config });
  ```

- **Example Migration (New Backend System):**

  ```typescript
  http.use(await createRouter({ logger, config }));
  ```

- **Internal Refactoring:** The internal sandbox directory `packages/app-next` has been renamed to `packages/dev-sandbox`.
- **Related PR:** Ref commit [91c8dfc](https://github.com/immobiliare/backstage-plugin-gitlab/commit/91c8dfc75647a32da950a8585d8556a7982da14e).
